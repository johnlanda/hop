...
| [messagingscenarios_helpers_test.go](messagingscenarios_helpers_test.go) | `managerEnv`, `incarnationForSession`, `taskAddress`, `sessionQuestionTo`, `taskMailboxClosed`, `messageAcked`, `parseSentID`, `parseTaskID`, `requireByteExactFile`, `waitForObservation` | Slice 7b's one addition to the harness: driving a manager verb (`task create`/`plan close`/`msg send`) as a direct, test-issued one-shot process authenticated with the run's own live manager session identity (`managerEnv`, mirroring `answerHuman`'s "human" impersonation, generalized to the manager's `HOP_SESSION_ID`/`HOP_INCARNATION_ID`, itself read from the session's current `runtime_bindings` row) — lets a scenario construct an exact message/submission ordering deterministically and seed content the scripted manager-feature grammar's own whitespace-delimited, underscore-substituted TASK/ANSWER fields cannot carry losslessly. `parseSentID`/`parseTaskID` are test-side retypings of the embedded fixture's own `parseSentMessageID`/`parseCreatedTaskID` (compiled only into the separate fixture `claude` binary, not callable from this package's own test code); `requireByteExactFile`/`waitForObservation` are shared byte-exact-delivery assertion helpers. Shared by injectionfreedelivery_test.go and mailboxclosurerace_test.go |
| [injectionfreedelivery_test.go](injectionfreedelivery_test.go) | `TestRealProcessInjectionFreeDelivery`, `hostilePayload`, `extractBriefSection`, `sha256HexOf`, `terminalInputMethods`, `requireNoUnexpectedTerminalInputRequests` | Design section 11 scenario 7: the brief, a task's own instructions and a manager answer — driven directly as the manager, itself an otherwise-unscripted session, since the scripted manager-feature grammar cannot carry genuinely hostile bytes — are seeded with hostile content (ANSI escapes, a bracketed-paste open/close pair, `y` + newline, control characters, no NUL since the brief travels as a single argv element). Byte-exact delivery is asserted against the durable artifact file AND against the receiving fixture principal's own independent read (its observation dump) for the brief and the task instructions, and against the message's stored `body_digest` (sha256, cross-checked) for the answer. The disposable server runs with API request logging at the S11-pinned elevated level for the whole run (capture window bracketed by `pane.list`/`tab.list`, S11's own technique, spanning before `hop run` starts to after the controller exits); a positive control (`pane.send_text` on a dedicated throwaway pane, through a separate connection, exactly as a production subprocess connects) is asserted present; the capture is asserted to carry zero `api.request.complete`/`api.request.fail` entries for Herdr's complete real terminal-input method set (`pane.send_text`/`pane.send_keys`/`pane.send_input`/`agent.prompt`/`agent.send_keys`/`agent.start`) other than that one control |
| [mailboxclosurerace_test.go](mailboxclosurerace_test.go) | `TestRealProcessMailboxClosureRace`, `grammarTransientUndeliveredLine`, `grammarRefusalMailboxClosed` | The design section 11 "Layers" row's real-process proof of design section 5's mailbox closure/admission race, both orders constructed deterministically — never with a sleep, and never by racing live (the sqlite adapter suite already proves the true concurrent race from separate store handles) — instead: a message addressed to a task's mailbox the instant the task row exists, long before its own worker (`submit-valid`, a legacy Phase 2 behavior with no messaging awareness, so it never pre-drains) can possibly reach its own first `hop result submit`, is refused `transient: undelivered messages` on that first attempt and drained-then-resubmitted by the worker's own built-in retry logic (no message stranded, receipts both ways); a send addressed only once a DIFFERENT task's mailbox is already durably closed by its own accepted result (`worker-implement`) is refused `mailbox-closed`, no message row ever created. Plus the failure-path variant: a `worker-hold` task killed mid-attempt with its retry budget already exhausted (`RetryLimit=1`) fails directly (never a needs-rework detour, design section 5 reference trace 4's exhaustion variant), its failing settlement's snapshot-equality contract journaling an orphaned message — addressed to the still-open mailbox immediately after the kill, well ahead of the live controller's own scheduling-pass cadence — in the manager's own `task t1 failed` notice, the orphaned message never acknowledged, and a later send against the same, now failure-closed, mailbox refused |
