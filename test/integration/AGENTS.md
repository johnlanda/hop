...
| [reviewerrejection_test.go](reviewerrejection_test.go) | `verdictRejectedShortfallToken`, `TestRealProcessReviewerRejection` | Design section 11 scenario 5 (reference trace 5), written in full but `t.Skip`ped on defect STATUS-1 (reported and confirmed; tracked on a separate branch): `hop status -run` does not yet render `EvaluateReadiness`'s guard shortfalls at all, and no notice body or other channel names a reject verdict either (the acceptance notice's body is only the reviewer's own raw reasons text — confirmed no marker exists), so the fixture manager's standing hop-status-polling instruction (`statusReportsVerdictRejected`, embedded in `fixtureWorkerSource`) has nothing to read yet. The scenario itself: `reviewer-reject-once` rejects R1 without completing the run; asserts `hop status` names `verdictRejectedShortfallToken` (`run.ShortfallVerdictRejected`, `"verdict-rejected"`, mirrored here — retyped, never derived — from the identical embedded constant); the manager plans a fix task, whose integration moves the head; a new review task (`requireReviewTaskOtherThan`, distinct from R1) approves against the NEW head; the run completes; and R1's stale verdict (bound to the superseded head) is confirmed never able to satisfy the guard. Unskip by removing the one `t.Skip` line once STATUS-1 lands — no other change needed, since the fixture's own detection already matches the same kind token this scenario asserts |
...
Design section 11's exit-scenario row, this task's slice: `TestRealProcessFeatureRunEndToEnd` (scenario 1), `TestRealProcessWorkerInterruption` (scenario 4) and `TestRealProcessReviewerRejection` (scenario 5, written in full but `t.Skip`ped on defect STATUS-1 — see its own quick-reference row and `reviewerrejection_test.go`'s doc comment). Scenarios 2 (`RelayedQuestion`), 3 (`DuplicateAndAmbiguousDelivery`), 6 (`ControllerReconnectNoSecondClaimant`) and 7 (`InjectionFreeDelivery`), plus the additional messaging/controller/integration scenarios design section 11's "Layers" table names, are slices 7b/7c's on this tip — this task builds `featureharness_test.go` and the generalized fixture principal as their reusable base and does not implement them.

Defects found while implementing this slice, reported to and confirmed by the manager, tracked on separate branches:

- **STATUS-1**: `cmd/hop/statuscmd.go`'s `renderRunDetail` never renders `RunDetailView.GuardShortfalls`, `.Mailboxes`/`NeedsAttention` (the section 7 attention line), `.Tasks` or `.LatestIntegration`, despite `internal/app/usecase_status.go`'s `runDetailView` fully populating all four from the store — `hop status -run` therefore cannot show a reject-verdict guard shortfall, or anything else needed to detect one, even though the manager's role artifact/crib name no other channel for it (confirmed: neither the acceptance notice's body — the reviewer's own raw reasons text, sqlite/review.go's `persistVerdictAcceptance` — nor any other verb surfaces it). Reproducer: complete any feature run's review task with a reject verdict, then run `hop status -run <id>` — the shortfall never appears. Tracked on `phase-3/status-detail` (based on 50bcd11); blocks `TestRealProcessReviewerRejection` only. The fixture manager's own `statusReportsVerdictRejected` (embedded in `fixtureWorkerSource`) already polls `hop status` and matches `run.ShortfallVerdictRejected`'s `"verdict-rejected"` kind token per the manager's own standing instruction (design section 8), ready for STATUS-1 to land.
...
- `go test -count=1 -run 'TestFixtureManagerScriptDispatch|TestFixtureReviewerRejectOnce|TestFixtureWorkerHoldBarrier' -v ./test/integration` —
  Phase 3 slice 7a's isolated (no herdr) fixture principal unit coverage:
  the manager-feature scripted dispatch, reviewer-reject-once's one-shot
  marker across two invocations, and the worker-hold barrier, all driven
  directly against the fake hop stub (`fixtureprincipal_test.go`).
- `go test -count=1 -run 'TestRealProcessFeatureRunEndToEnd|TestRealProcessWorkerInterruption|TestRealProcessReviewerRejection' -v ./test/integration` —
  Phase 3 slice 7a's own real-process scenarios (`TestRealProcessReviewerRejection`
  reports `t.Skip: STATUS-1` until that defect lands — see "Phase 3 slice 7a
  real-process scenario coverage" above); both landed scenarios pay the same
  class of real wall-clock cost as the task 6b sweep below (concurrent
  worker scheduling, serial integration, a real review round trip).
