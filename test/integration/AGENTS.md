...
| [spike_worktree_test.go](spike_worktree_test.go) | `TestSpikeConcurrentWorktreeCreate`, `TestSpikeWorktreeCreateExistingBranchIgnoresBase`, `worktreeCreatedResponse`, `workspaceGet`, `callWorktreeCreateAsync`, `worktreeHead` | Phase 3 spike S9: two concurrent `worktree.create` calls against one repository, each pinning `--base <oid>` honored for a new branch (the checkout's HEAD equals the requested base), the `{workspace, tab, root_pane, worktree}` response shape, a distinct workspace/path/branch per call, and repo_key-based grouping with the parent workspace (the manager's own `workspace.create`d workspace, which worktree.create resolves and marks as the non-linked source); a created worktree workspace coexists with an S6 command-pane launch at its own checkout path. A second test names a FINDING: an already-existing, not-checked-out-elsewhere branch is checked out at its own current tip, silently ignoring `base` |
...
| [spike_gitrefs_test.go](spike_gitrefs_test.go) | `TestSpikeIntegrationRefCompareAndSwap`, `TestSpikeDetachedMergeLeavesRefUntouched`, `runGit`, `mustRunGit`, `readRef` | Phase 3 pre-freeze git-level probes (provisional numbering, requested by the design reviewer for the fenced-publish assumptions a revised integration design rests on): `git update-ref <ref> <new> <old>` against a `worktree.create`'d checkout is a true compare-and-swap (succeeds and moves the ref when `<old>` matches; refuses and leaves the ref untouched when stale); a `git merge --no-ff` run in a plain DETACHED scratch checkout (never the integration branch's own checkout) computes a candidate merge commit without moving the run-scoped ref at all — publishing is a separate, explicit CAS step |
...
- `go test -count=1 -run TestSpike ./test/integration` — the Phase 2 capability
  ...
  `TestSpikeWorktreeCreateExistingBranchIgnoresBase`,
  `TestSpikeMultiPaneEnvIsolation` — plus the provisional git-ref probes
  `TestSpikeIntegrationRefCompareAndSwap`,
  `TestSpikeDetachedMergeLeavesRefUntouched`). S4 is skipped
