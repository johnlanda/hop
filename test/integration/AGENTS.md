...
- `pane.process_info` foreground members carry NO positional semantics,
  ...
  is ready, so the worker is ordinarily NOT index 0 on macOS (the raw
  order observably lists children first; the live probe saw an MCP
  server at index 0). Scenarios locate the worker by the launch claim's
  recorded pid via `fixtureRun.workerForegroundPID`, and
  `TestRealProcessSettlementWithMCPGroupMembers` pins the shape.
...
group with the claimed pid not at index 0
