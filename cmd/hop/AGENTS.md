...
| [launchcmd.go](launchcmd.go) | `runLaunch` | `hop launch --run --attempt`, the worker exec boundary: worker state root, `PrepareLaunchExec` (validate → sanitize → compose → resolve → claim), `process.Exec`; a post-claim failure settles exec_failed via `FailLaunchExec`; one stderr line, never an environment value |
