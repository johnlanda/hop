...
| [launchcmd.go](launchcmd.go) | `runLaunch`, `launcherWorkerDir`, `resolveCanonicalPath` | `hop launch --run --attempt`, the worker exec boundary: worker state root, the launcher's own symlink-resolved working directory (refused with a diagnostic before any store access when unresolvable), `resolveCanonicalPath` as the request's `ResolvePath` seam for the recorded-worktree cross-check whose resolved directory is the workspace-trust seed key, `PrepareLaunchExec` (validate → sanitize → per-harness compose — all three harnesses launch, cold resume is Claude-only — resolve → worktree cross-check → trust-seed → claim), `process.Exec`; a post-claim failure settles exec_failed via `FailLaunchExec`; one stderr line, never an environment value |
