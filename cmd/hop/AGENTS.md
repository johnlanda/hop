...
| [grammarcontract_status_test.go](grammarcontract_status_test.go) | `TestGrammarContractStatusFeatureDetailSections` | Drives `hop status -run`'s STATUS-1 feature-mode detail block through the built binary: a real `hop review submit --verdict reject` (verdict-rejected, no task), a seeded implement task (task-not-integrated, with one), a real `hop msg send` left queued (the attention line), and a pending human question with its exact `hop answer` invocation; crossing the attention threshold to reach the nested action line and the `blocked, needs attention` marker is left to the in-process unit tests (`cmd/hop`'s `TestRunStatusFeatureDetailBlock`, `internal/app`'s `TestStatusMailboxesAndAttention`) since this suite has no seam to fast-forward the exec'd binary's own clock |
