...
| [launchcmd.go](launchcmd.go) | `runLaunch`, `launcherWorkerDir` | `hop launch --run --attempt`, the worker exec boundary: worker state root, the launcher's own symlink-resolved working directory (the workspace-trust seed key, refused with a diagnostic before any store access when unresolvable), `PrepareLaunchExec` (validate → sanitize → per-harness compose — all three harnesses launch, cold resume is Claude-only — resolve → trust-seed → claim), `process.Exec`; a post-claim failure settles exec_failed via `FailLaunchExec`; one stderr line, never an environment value |
