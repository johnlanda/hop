...
| [featureloop.go](featureloop.go) | `featureCheckDriver`, `runFeatureControllerLoop`, `runFeatureSchedulingPass`, `describeFeatureCheckReport`, `finishFeatureControllerLoop` | The feature-mode foreground loop: one scheduling pass per tick (`RetireSettledSessions` → `RecomputeReleases` → `DriveIntegration` → `EnsureReviewTask` → `DriveCompletion` → `AssignReadyTasks` → `CorroborateSessionLaunches`), gated to `CorroborateSessionLaunches`-only while the run is launching (no settled manager session yet to schedule against), plus async `DriveFeatureChecks` via `featureCheckDriver` (mirrors `checkDriver`'s solo shape); `runFeatureSchedulingPass` populates every `AssignReadyTasks` field itself each pass (`AssignmentDefaults` for the frozen ones, the loop's own already-resolved `RepositoryRoot`/`HOPPath`/`StateRoot`, `ResolveIntegrationHead` for `IntegrationHeadCommitOID`) |
