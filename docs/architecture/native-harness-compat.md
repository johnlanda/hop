...
fresh `HOME` or fresh `XDG_DATA_HOME`/`XDG_CONFIG_HOME` pair.
...
home still failed with 401, ignoring the same variable. The Claude Code
probes below were run with `ANTHROPIC_API_KEY` explicitly unset; its binary
also adopts `CLAUDE_CODE_OAUTH_TOKEN` from the environment (code inspection).
HOP must launch harnesses with provider credential variables removed unless
an explicit binding says otherwise.
...
- Profile isolation. With `CLAUDE_CONFIG_DIR` set to an empty directory,
  ...
  does not exist. An isolated profile therefore does not inherit the global
  account (v2.1.259).
...
- Credential storage (code inspection, installed binary
  `/opt/homebrew/Caskroom/claude-code@latest/2.1.259/claude`). Credentials are
...
- Onboarding. A fresh profile's first interactive launch renders a theme
  ...
  that looks best with your terminal`, captured through a PTY) before any
  prompt can be delivered. `.claude.json` carries `hasCompletedOnboarding` and
  `lastOnboardingVersion` keys.
...
1. Create `<dir>` (once per account) and pre-seed `<dir>/.claude.json` with
   `hasCompletedOnboarding`, a `theme`/onboarding choice and a `projects`
   entry for the workspace root (or an ancestor) with
   `hasTrustDialogAccepted: true`, so the first interactive launch does not
   block. Values to pre-seed are an open question below; print-mode launches
   need no trust pre-seeding.
2. Perform the account login once, interactively, in that profile
   (`CLAUDE_CONFIG_DIR=<dir> claude` then `/login`); this is a human step.
3. Create the pane/tab with `env` containing `CLAUDE_CONFIG_DIR=<dir>` at
   creation time — `agent.start` has no env parameter (see
   [Herdr surface](herdr-surface.md)) — then `herdr agent start <name>
...
- Profile isolation. With `CODEX_HOME` set to an empty directory,
  ...
  confirming the global credential was never read (v0.154.0).
...
- Credential storage. Login state is a plain `auth.json` file inside
  `CODEX_HOME` (structure above), so two homes hold accounts independently and
  refresh (tracked by `last_refresh`) rewrites only the owning home's file.
  `codex login` supports a browser flow, `--device-auth`, and non-interactive
  `--with-api-key` / `--with-access-token` reading from stdin (help text).
...
- No login was performed in an isolated home, so account-holding and refresh
  independence across two homes are structural facts (separate files) awaiting
  an authenticated confirmation with a second account.
...
1. Create `<dir>` (once per account) and pre-seed `<dir>/config.toml` with a
   `[projects."<workspace-root>"]` `trust_level = "trusted"` entry, the chosen
   `model`/`approval`/`sandbox` defaults and any MCP servers, so the first
   interactive launch does not block. Equivalent `-c` overrides can be passed
   per launch instead.
...
3. Create the pane/tab with `env` containing `CODEX_HOME=<dir>` at creation
   time, then `herdr agent start <name> --kind codex --pane <pane>`.
...
- Credential storage. `opencode auth list` prints the credential file path
  ...
  probed until a login exists. Docs: `auth.json` holds "API keys, OAuth
  tokens"; opencode refreshes OAuth tokens itself for the providers that
  document it; Anthropic Claude Pro/Max subscription OAuth is prohibited and
  its plugins were removed as of 1.3.0, so Anthropic access is API-key based.
...
1. Choose the profile shape: `HOME=<dir>` when the Herdr opencode integration
   must be installed per profile; otherwise `XDG_DATA_HOME=<dir>/data` and
   `XDG_CONFIG_HOME=<dir>/config` suffice for credentials, sessions and
   config. Strip provider API-key variables (`OPENAI_API_KEY`,
   `ANTHROPIC_API_KEY`, `OPENAI_BASE_URL`, …) from the launch environment.
...
3. Create the pane/tab with that env at creation time, then
...
- The cold-restore path constructs its launch environment as
  ...
  already noted by the [Herdr surface audit](herdr-surface.md). Herdr's own
  auto-resume therefore launches the resume command without
  `CLAUDE_CONFIG_DIR`, `CODEX_HOME` or the opencode `HOME`/XDG overrides.
...
profile resolves against the default global profile: the transcript is not
found there, and any session it did find would run on the global account (or,
for opencode, on whatever provider key the restore environment carries).
Auto-resume of isolated-profile sessions is account-incorrect by construction
in 0.9.0.
...
| Cold resume by HOP: relaunch with the recorded profile env, same account, recorded cwd | Supported, pending one opt-in authenticated smoke per harness | Resume lookup verified unauthenticated for Claude Code and Codex; full unauthenticated resume round-trip verified for opencode on its free model; authenticated continuation untested everywhere |
...
Per [plugin testing](plugin-testing.md) layer 5, one Go test package (proposed
`test/live`, build tag `live_harness`, skipped unless
`HOP_LIVE_HARNESS=1`) should, for each harness: create a temporary profile
directory with provider API-key env vars stripped; assert the
unauthenticated probe behaves as recorded here (Claude Code and Codex fail
with their not-logged-in errors; opencode answers on its free model and
`opencode auth list` reports zero credentials); then, only when the human has
pre-authenticated the directory named by `HOP_LIVE_CLAUDE_PROFILE` /
`HOP_LIVE_CODEX_HOME` / `HOP_LIVE_OPENCODE_PROFILE`, run one one-word prompt,
record the session id, kill the process, resume by id with the same profile
env and assert the transcript continues, and assert the global
`~/.claude.json`, `~/.codex/auth.json` and
`~/.local/share/opencode/auth.json` modification times are unchanged (or the
file remains absent). It must never write outside its temporary directories
and never store or log token values. The ordinary test suite must not depend
on it.
...
- own one directory per account profile and produce launch specifications
  whose env sets exactly `CLAUDE_CONFIG_DIR`, `CODEX_HOME` or the opencode
  `HOME`/XDG overrides — and removes provider API-key variables — applied at
  pane/tab creation because `agent.start` accepts no env;
- pre-seed onboarding and trust configuration inside the profile before first
  interactive launch, and report a profile that would block on login or
  onboarding as not launch-ready;
