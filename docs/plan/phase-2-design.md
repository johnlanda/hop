...
| `runtime_bindings` | id PK, session_id FK, incarnation_id, server_socket_path, server_instance NULL (the opaque, adapter-formatted identity of the server process behind the configured socket at binding creation — the herdr adapter formatted it from the socket peer pid, e.g. `peer-pid:42111`, and since Phase 3 formats one server LIFETIME, the peer pid plus that process's start time, e.g. `herdr-server-lifetime/v1 pid=42111 start=1789614609.080021`, so a restarted server reusing the pid never repeats it — docs/plan/phase-3-design.md section 3; NULL/empty means unknown; compared by equality only for the continuity checks, and a token of the earlier format never equals a current one), workspace_id, tab_id, pane_id, creation_label, occupant_evidence JSON NULL (label + argv marker + pid; never pid alone), launch_kind (initial, resume, restored-observed), observed_at, superseded, superseded_evidence NULL | append-only; UNIQUE(session_id, incarnation_id) |
