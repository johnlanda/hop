...
(`app.MessagingStore`, `app.PlanStore`, `app.ReviewStore`), the
controller unit of work's `app.WorktreeRepository` and the
...
| --- | --- | --- |
| [storevectors.go](storevectors.go) | `TaskCreateSelfDependency`, `TaskCreateRequestIDConflictFirst`/`Second`, `TaskCreateNonManagerCaller`, `TaskCreateOversizedTitle`, `AckMessageStaleIncarnation`, `AckMessageUnknownMessage`, `AckMessageNotDelivered`, `MessageSendAnswerUnknownQuestion`, `MessageSendAnswerNotRecipient`, `MessageSendAnswerMailboxClosed`, `MessageSendClaimedAddressMismatch`, `MessageSendClaimedAddressReplay`, `MessageSendCrossRun`, `MessageFetchCrossRun`, `AckMessageCrossRun`, `MessageFetchSupersededAttempt`/`MessageFetchSupersededAttemptDetail`, `AckMessageSupersededAttempt`/`AckMessageSupersededAttemptReason`, `MessageFetchEndedManager`/`MessageFetchEndedManagerDetail`, `AckMessageEndedManager`/`AckMessageEndedManagerReason`, `MessageSendSupersededAttempt`/`MessageSendSupersededAttemptReason`/`MessageSendSupersededAttemptDetail`, `MessageSendEndedManager`/`MessageSendEndedManagerReason`/`MessageSendEndedManagerDetail`, `ReviewSubmitForeignReviewer`/`ReviewSubmitForeignReviewerReason`/`ReviewSubmitForeignReviewerDetail`, `TaskRetry`, `WorktreeCreateUnknownAttempt`, `WorktreeCreateForeignAttempt`, `WorktreeVectorBaseCommit`, `WorktreeLookupVector`, `WorktreeLookupByAttempt` | Refused-input vectors plus one accepted-shape vector (`TaskRetry`: accepted and its identical replay as duplicate, both carrying the retried task's `TaskSeq` and the reserved `AttemptNumber` that `hop task retry` renders into the shared grammar lines). The refusal vectors: a malformed dependency graph, a reused request-ID with conflicting content, a non-manager caller, an oversized title, a stale acking incarnation, an answer replying to an unknown question, an answer from a current session that is not the question's recipient (refused `unauthorized`, the question left unacknowledged), an answer from the recipient into a closed task mailbox (refused `mailbox-closed`, nothing inserted), a send of any kind from a current session claiming a logical address its session row does not resolve to — a worker claiming `manager` or another task, or a session with no messaging role (refused `unauthorized`, detail pinned, since it shares the token with the answer-authority refusal), the same claim replaying the claimed address's accepted request ID with the same or another body (the same refusal, never `duplicate` or `conflicting`), a send/fetch/ack from a session belonging to a DIFFERENT run than the request claims, a fetch and an ack from a session that is no longer its address's current session — a task's retired attempt session (attempt 1 interrupted, the session terminated with its binding current, attempt 2 running: the fetch refused `app.ErrMessagingUnauthorized` with its detail pinned and nothing served, the ack of a message it was served refused `stale` with no ack row) and a terminated manager whose binding is current beside its successor (the same two refusals), a send from either of those sessions — the retired task session's answer to a question it was served, the ended manager's info to a task — (refused `stale` with the fetch refusal's detail pinned and no envelope, the current session's own send then accepted) — and a review submission from a live reviewer that is not the claimed attempt's own (another run's reviewer, or another review attempt's — both fixture shapes share the one vector; refused `not-reviewer` with its detail pinned) — one function per vector, each returning the exact `app.TaskCreate`/`app.MessageAck`/`app.MessageSend`/`app.MessageFetch`/`app.ReviewSubmission` value to pass to the port method its doc comment names. The two worktree vectors are `run.Worktree` values for a unit of work's `Worktrees().Create` under the named run's own lease, well-formed except for the attempt link: an unknown attempt (refused `app.ErrNotFound`) or another run's attempt (refused `app.ErrFenced`), with no row written either way. `WorktreeLookupByAttempt` is the `app.WorktreeIndexRepository.ByAttempt` contract: four rows (an unlinked solo-shaped row, an older and a newer row linked to the target attempt, a sibling attempt's row) inserted in order, then the newest linked row answering for each linked attempt and `app.ErrNotFound` for a row-less attempt and for the empty attempt |
...
- Consumed/implemented ports: none — this package builds `app.TaskCreate`/
  ...
  `run.Worktree` values for `app.WorktreeRepository`, and
  `app.NewRunSpec` values for `app.StateStore.InitializeRun`, and the
  `app.WorktreeIndexRepository` lookup vector; it implements none of them.
