...
| [review.go](review.go) | `ReviewStore`, `ReviewSubmission`, `ReviewOutcome`/`ReviewOutcomeKind` | Section 8's worker-authority review-verdict port and its DTOs (`ReviewOutcome.Transient` names a retryable verdict's typed reason); the accepting transaction's guard/retirement wiring is driving-controller behavior owned by a later slice |
...
| [usecase_review.go](usecase_review.go) | `SubmitReviewVerdict`, `SubmitReviewRequest`/`Result`, `ReviewReasonsLimit` | The driving use case behind `hop review submit` (carrying the verdict's typed transient reason as `SubmitReviewResult.TransientReason`, under the same `checkTransientReason` contract as results): parse and bound (verdict token, reasons ≤ 64 KiB), resolve the subject TREE object ID against the recorded repository before the accepting transaction, write the reasons artifact file-first, delegate to `ReviewStore.SubmitReview` |
