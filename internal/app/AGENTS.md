...
- The Phase 2 run-controller scenario suite runs against `fakeStore` (one
  ...
  1 MiB per-stream capture bound to every answer, scripted, hooked or
  modeled (`boundCapturedOutput`), and computes the truncation flags
  itself; an answer claiming a truncation the Runner could never report
  (fewer bytes than the bound) is refused
...
- `go test ./internal/app -run TestRemoveRunWorktrees` — the removal
  ...
  - **A cut post-act listing:** after a refused act, a worktree listing
    cut by the capture bound before the checkout leaves the operation
    reconciling and the row active, never released.
...
- `go test ./internal/app -run TestRetireWorktrees` — the pass driver:
  ...
  - **A cut worktree listing defers the run:** with the listing reaching
    the capture bound exactly at the record boundary before the first
...
- `go test ./internal/app -run TestRecoverWorktreeRetire` — the
  ...
  - **Blocked across passes, never re-dispatched and never removed:** a
    ...
    can be observed, and a worktree listing cut by the capture bound
    before the checkout's record (never released as unregistered).
...
- `go test ./internal/app -run TestInspectAttemptCheckout` — the pre-act
  ...
  non-canonical recorded path. Output past the capture bound is never
  decided on: an assume-unchanged or skip-worktree flag beyond the kept
  prefix (cut exactly on an entry boundary, or inside an entry), a clean
  index past the bound, a truncated stderr, a status read past the bound,
  a checkout common-directory read whose prefix names another existing
  repository, and a worktree listing cut on the record boundary before
  the candidate (present or gone) are all retained as inspection-failed,
  while a large index or listing that fits the bound exactly is still
  decided normally. Every git read must be the absolute git under the
