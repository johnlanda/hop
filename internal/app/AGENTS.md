...
- `go test ./internal/app -run TestInspectAttemptCheckout` — the pre-act
  ...
  inspection-failed. So is a listed record spelled exactly as the
  candidate's canonical path that no longer resolves (present or gone),
  while an unrelated uninspectable record is skipped. Only the index scan
