...
| [messaging.go](messaging.go) | `MessagingStore`, `MessageSend`, `MessageFetch`, `MessageDelivery`, `MessageAck`, `MessageOutcome`/`MessageOutcomeKind` (incl. `MessageTransient`), `MessageAckOutcome`/`MessageAckOutcomeKind`, `HumanAnswer`, `AddressString`, `MessageBodyFileLimit`, `MessageBodyInlineLimit`, `ErrMessagingUnauthorized` | Section 7's worker-authority messaging port and its DTOs: send/fetch/ack/answer, request-ID idempotency, relay provenance, the `origin` resolution for a forwarded human answer. Only an ordinary (question/info) send is run-state-gated, by the same `CanAcceptManagerVerb` split as the plan verbs (`MessageTransient` or `MessageRunNotAccept`); fetch, ack and an answer carry no run-state gate. Every method independently re-derives the caller session's own run (and, for fetch, its current incarnation and resolved address) rather than trusting the request's caller-supplied fields — `ErrMessagingUnauthorized` is `FetchNextMessage`'s refusal signal (it has no outcome-kind field); `SendMessage`/`AckMessage` express the identical check through their own outcome kind (`MessageRefused`/`AckRefused`) |
