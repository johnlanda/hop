...
| [messaging.go](messaging.go) | `MessagingStore`, `MessageSend`, `MessageFetch`, `MessageDelivery`, `MessageAck`, `MessageOutcome`/`MessageOutcomeKind` (incl. `MessageTransient`), `MessageAckOutcome`/`MessageAckOutcomeKind`, `HumanAnswer`, `AddressString`, `MessageBodyFileLimit`, `MessageBodyInlineLimit`, `ErrMessagingUnauthorized` | Section 7's worker-authority messaging port and its DTOs: send/fetch/ack/answer, request-ID idempotency, relay provenance, the `origin` resolution for a forwarded human answer, fetch and ack only by the address's current session (a manager session that has not ended; for a task address, the session of the task's newest, non-terminal attempt — a fetch from any other session is `ErrMessagingUnauthorized`, an ack refused `stale`), and the answer rules — only the question's recipient (its address re-derived by the store) may answer, and a closed task destination refuses a first answer. Only an ordinary (question/info) send is run-state-gated, by the same `CanAcceptManagerVerb` split as the plan verbs (`MessageTransient` or `MessageRunNotAccept`); fetch, ack and an answer carry no run-state gate. Every method independently re-derives the caller session's own run (and, for send and fetch, its resolved address; for fetch, its current incarnation too) rather than trusting the request's caller-supplied fields — a send whose `SenderAddress` is unresolvable or disagrees is refused `unauthorized` before any addressing decision — `ErrMessagingUnauthorized` is `FetchNextMessage`'s refusal signal (it has no outcome-kind field); `SendMessage`/`AckMessage` express the identical check through their own outcome kind (`MessageRefused`/`AckRefused`) |
...
- Phase 3 scenario suite, against the same `fakeStore` extended with
  ...
  - `go test ./internal/app -run TestFakeAnswer` — `fakeStore`'s half of
    ...
    (request-ID receipt, then authority, then prior content); an answer
  ...
  - `go test ./internal/app -run 'TestFakeFetchRequiresTheTaskCurrentAttemptSession|TestFakeAckRequiresTheTaskCurrentAttemptSession|TestFakeManagerMessagingRequiresAManagerSessionThatHasNotEnded'` —
    ...
    fetch and ack decide the address's current session through
