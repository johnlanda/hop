...
| [usecase_restartlifetime.go](usecase_restartlifetime.go) | `ReconcileServerRestart`, `RestartReport`, `RestartOptions`, `restartIdentity`, `restartDisposition`, `identifyRestartedPane`, `restartRecordedPID`, `findRestartClose`, `openRestartClose`, `dispatchRestartClose`, `finishRestartClose`, `restartDispositionFor`, `applyRestartDisposition`, `restartInterruption`, `settleRestartLaunchEnded`, `restartCandidateSessions`, `restartCloseEvidence`, `restartSessionReason`, `restartRelaunchReason`, `launchEndedRestartReason`, `RestartDispositionFor`, `RestartDispositionClosed`, `RestartDispositionRelaunched` | The restart-lifecycle step (docs/plan/phase-3-design.md sections 4 and 6): on a CHANGED server lifetime — `ClassifyServerLifetime`'s positive verdict, read BEFORE and AFTER every observation and required to agree with itself, so observations spanning a restart conclude nothing — HOP closes the panes it owns and cold-relaunches those sessions from their recorded native references. CHANGED replaces the CONTINUITY conjunct ONLY: the recorded process (the claim's pid, else the corroborated occupant's) must still be observed gone by a successful group listing, exactly as before. The close never goes by recorded pane id alone, because a pane id is not a durable address across a restart — a workspace closed before one leaves its id free, the restarted server reissues it, and the composed pane id answers for somebody else's pane (`TestSpikeRecordedPaneIDCanAddressADifferentPane` observed that collision). `identifyRestartedPane` is the ladder: the creation label resolving to exactly the recorded id (a layout attribute, so it speaks in BOTH restore windows), else exactly one foreground member running this session's own native restore invocation (`MatchRestoredHarness`, which survives a human's rename); anything else — a label answering another pane, a failed lookup, an unclaimable occupant, two restore members, or neither address answering — closes nothing and reports a value-free reason. `MatchRestoredHarness`'s None outcome is not suspicious and never fail-closed on its own: it is the ordinary answer for a pane whose restore has not fired. The close journals its own `pane.close` intent under `closeReasonRestart`, freezing the OBSERVING lifetime (the placement's is gone), so the absence confirmation brackets against the lifetime that serves the socket now; `findOrCreateCloseOperation` defers to it rather than adopting one. `restartDispositionFor`'s five branches: a held stop or terminal failure, and every solo run, RETIRE (the close IS the retirement — this is the branch that fixes the reported defect, where the run otherwise stays `stopping` forever and `observeWorkerExit` never fires); an unsettled claim settles `exec_failed` under `launchEndedRestartReason`, a third recognized launch-ended reason, because a pre-assigned native reference names no transcript for a harness that never started (`TestSpikeClaudePreassignedSessionID`); a settled claim with a Claude harness and a native reference is COLD RELAUNCHED through the existing `coldRelaunchFeatureSession` (never a second path), with the manager reconciled LAST so a round that fails partway has not moved the run's lineage; anything else is interrupted and the manager decides. Solo runs take this step only under a held stop, since solo cold relaunch stays exclusively a `hop resume` action. `RestartDispositionFor` is the ONE mapping from this rule's journaled reasons to what `hop status` renders: the read model reduces a session's newest transition reason through it, so the journal keeps the detail and the surface carries only the two fixed value-free strings |
