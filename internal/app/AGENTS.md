...
| [runtime.go](runtime.go) | `Runtime`, `ErrPaneNotFound`, `WorktreeRequest`/`WorktreeInfo`, `WorkerPaneRequest`, `PaneHandle`/`PaneRef`, `ProcessInfo`, `PaneProcess`, `WorkspaceRuntime`, `WorkspaceRequest`/`WorkspaceHandle`/`WorkspaceRef` | Herdr's pane and worktree surface: `CreateWorktree`, `OpenWorkerPane` (a `layout.apply` command pane), `FindPaneByLabel` recovery, `ReadPane` evidence, `InspectPane` occupant identity (reporting the typed `ErrPaneNotFound` for a pane that positively does not exist — distinct from any inspection failure), `ClosePane` (a pane id the server does not have is `ErrPaneNotFound`: nothing was closed), and `ServerInstance` — the opaque identity scoped to both the configured socket and the server process behind it, compared by equality only. `SendText` (the fixed-grammar launch-line fallback transport) was deleted in slice 6: HOP delivers everything by pull and never had a call site for it. `WorkspaceRuntime` (manager placement, S8-pinned shapes) is a SEPARATE interface from `Runtime`, consumed through the optional `Controller.Workspaces` field, for the same reason `WorkflowRepositories` is separate from `UnitOfWork` |
...
| [usecase_launchended.go](usecase_launchended.go) | `observeLaunchEnded`, `settleIfLaunchEnded`, `observeClaimedProcessGone`, `launchEndedReason`, `launchEndedByController`, `claimedProcessLiveDetail`, `observeUnplacedLaunchEnded`, `settleIfUnplacedLaunchEnded`, `launchEndedUnplacedReason`, `unplacedLaunchEndedOutcome`, `launchEndedResolution`, `launchEndedIntentLocked`, `unplacedLaunchLiveDetail` | The launch-ended decision row (docs/plan/phase-3-design.md section 4): an `exec_pending` claim whose own current, unsuperseded binding's pane is positively absent (`observePaneAbsence`) AND whose claimed process is gone from its own process group settles `exec_failed` under the lease with the fixed reason and the pane and pid as evidence; everything short of that pair is ambiguous. Its label-only variant, for a launch whose pane.open outcome (and so binding) was never recorded: no binding, exactly one unresolved decodable labeled pane.open naming the session (nothing unusable that might), that intent's own `exec_pending` claim, a successful creation-label lookup finding nothing and the claimed process gone settle the claim `exec_failed` with the distinct `launchEndedUnplacedReason` (pid as evidence) AND the pane.open `failed` with `unplacedLaunchEndedOutcome` (dispatched, never refused), in one transaction that re-reads the facts; `launchEndedIntentLocked` lets `sessionLaunchClaimLocked` keep reading that claim through the resolved intent. Corroboration, resume, stop and the terminal-failure shutdown all decide by both |
...
- Slice 2b scenario suites (`go test ./internal/app -run '<name>'`):
  ...
  claimed process — an unknown instance, a recorded id gone with the
  label answering, another process in the pane, an empty foreground and
  a failed pane.open each keep the run resuming) and
  `TestResumeFeatureChildUnplacedLaunch` (adoption by label then in
  flight; label and process gone settled through the label-only row;
  a live process pending with the action named);
  ...
  superseded binding), `TestLaunchEndedRowFailsTheRunForTheManager`,
  ...
  answering — bound instead — and a binding committed between the
  observation and the settlement), with the manager's unbound
