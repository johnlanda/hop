...
| [presentation.go](presentation.go) | `AgentPresentation`, `Presenter`, `AgentDisplay`, `PaneMetadata`, `ViewSelection`, `Role`, `SortDisplays`, token constants | Consumer-owned presentation port and its ordering: a display's manager-first `hop_order` key and padded token map, and the view selection HOP installs. `ReportMetadata` reports a pane the server no longer has (or one with no terminal) as an error wrapping `ErrPaneNotFound` — never absence evidence for a lifecycle decision |
...
| [messaging.go](messaging.go) | `MessagingStore`, `MessageSend`, `MessageFetch`, `MessageDelivery`, `MessageAck`, `MessageOutcome`/`MessageOutcomeKind` (incl. `MessageTransient`), `MessageAckOutcome`/`MessageAckOutcomeKind`, `HumanAnswer`, `AddressString`, `MessageBodyFileLimit`, `MessageBodyInlineLimit`, `ErrMessagingUnauthorized` | Section 7's worker-authority messaging port and its DTOs: send/fetch/ack/answer, request-ID idempotency, relay provenance, the `origin` resolution for a forwarded human answer, send, fetch and ack only by the address's current session (a manager session that has not ended; for a task address, the session of the task's newest, non-terminal attempt — a fetch from any other session is `ErrMessagingUnauthorized`, a send of any kind or an ack refused `stale`), and the answer rules — only the question's recipient (its address re-derived by the store) may answer, and a closed task destination refuses a first answer. Only an ordinary (question/info) send is run-state-gated, by the same `CanAcceptManagerVerb` split as the plan verbs (`MessageTransient` or `MessageRunNotAccept`); fetch, ack and an answer carry no run-state gate. Every method independently re-derives the caller session's own run and its current incarnation (and, for send and fetch, its resolved address) rather than trusting the request's caller-supplied fields — a send whose `SenderAddress` is unresolvable or disagrees is refused `unauthorized` before its request-ID receipt is read — `ErrMessagingUnauthorized` is `FetchNextMessage`'s refusal signal (it has no outcome-kind field); `SendMessage`/`AckMessage` express the identical check through their own outcome kind (`MessageRefused`/`AckRefused`) |
| [plan.go](plan.go) | `PlanStore`, `TaskCreate`/`TaskCreated`, `RetryRequest`/`RetryAccepted`, `PlanClose`/`PlanCloseResult`, `WorkflowOutcomeKind` (incl. `WorkflowTransient`), `RunNotRunningDetail`, `TaskTitleLimit`, `TaskInstructionsLimit` | Section 8's worker-authority manager-plan port and its DTOs: `hop task create`/`hop task retry`/`hop plan close`, each manager-only and run-state-gated inside its own transaction by `run.Run.CanAcceptManagerVerb` after the caller's authority and incarnation: `WorkflowTransient` (empty reason, `RunNotRunningDetail`'s state-only detail, a `transient` receipt outside the acceptance key) while the run can still reach running, `WorkflowRefused` with `run-not-accepting` once it never will. `RunNotRunningDetail` is the one detail text every store and fake sets on a transient outcome, the message one included |
| [review.go](review.go) | `ReviewStore`, `ReviewSubmission`, `ReviewOutcome`/`ReviewOutcomeKind`, `ReviewNotReviewerDetail`, `ReviewRefusalReasonOf`, `ReviewReasonAdmitted` | Section 8's worker-authority review-verdict port and its DTOs (`ReviewOutcome.Reason` is a refused verdict's grammar token, set where the store decides — `subject-mismatch` through `ReviewRefusalReasonOf` for `run.ErrVerdictSubjectMismatch`, `not-reviewer` for a caller that is not the attempt's reviewer session, `stale` for every other ineligibility, `malformed`, `conflicting` — and `ReviewReasonAdmitted` is the one kind-to-token table; `ReviewOutcome.Transient` names a retryable verdict's typed reason); the accepting transaction's guard/retirement wiring is driving-controller behavior owned by a later slice |
| [readstore.go](readstore.go) | `ReadStore`, `RunStatus`, `RunDetail`, `CheckExecutionSummary`, `FrozenRun`, `CheckExecutionContext`, `TaskSummary`, `IntegrationSummary`, `MailboxStatus`, `InFlightMessage`, `PendingQuestion`, `SessionSummary`, `StatusGuardShortfalls`, `RecordedSubject`, `ShortfallEvidenceInconsistent`, `recordedTree` | Lease-free reads for `hop status` and both exec boundaries. `RunDetail` carries the identities stop and resume need, the frozen `StateRoot` for evidence capture, and `LastCheck` (the newest check execution's identity, state and retained evidence); `FrozenRun` is the check use case's frozen-input source. Phase 3 additions to `RunDetail`, all additive fields, empty for a solo run: `Tasks` (the feature-mode task table), `LatestIntegration` (the run's most recently CREATED integration row — reading exactly what is recorded, never resolving the live git ref, which is 2b's integration-operations concern), `GuardShortfalls` (`StatusGuardShortfalls`' list, which both stores call: `EvaluateReadiness`'s missing list rendered verbatim, its head tree the one the run's recorded subjects — review tasks' frozen subjects and accepted reviews' subjects, each resolved from git — name for exactly the head commit, never the commit id; with no such record the head tree stays empty, which can report a review only stale and a check only missing, never current; when the records name different trees the check and verdict shortfalls are replaced by the read-model-only, value-free `ShortfallEvidenceInconsistent` while the plan and task guards stay), `Mailboxes` (section 7's per-address queue-depth/in-flight-age status surface, `Attention` computed once here against the frozen `[messages] attention_after` threshold), `PendingQuestions` and (STATUS-1) `Sessions` — every session the run has ever created, its role, state, current binding, (for a delegated child) the task and attempt number it is bound to, and `LaunchCorroborationPending`, the structural fact behind the session's own action line (reconciling, with a current unsuperseded placed binding whose incarnation's launch claim is still `exec_pending` — the one reconciling state the launch corroboration re-inspects, false for every other session); a plain additive struct field (unlike the packaging rule below, which is about widening a PORT), so `LoadRunStatus`'s existing signature and every other `ReadStore` implementer are untouched. `RunDetail.TargetBranch` and `RunDetail.WorktreesRetiredAt` are the frozen worktree-retirement target and the run's worktrees-retired fact (nil until set); a feature run's `RunDetail.Worktrees` (every row, oldest first) and `RunDetail.WorktreeRetirements` (its `worktree.retire` operations, newest first) are the per-row status lines' input, both nil for a solo run. Slice 6 deleted the Phase 2 run-keyed `LoadLaunchContext`/`LaunchContext` in favor of `WorkflowReadStore.LoadSessionLaunchContext` — `hop launch` is session-addressed for every role now, the permanent solo shim included. `RunStatus.NeedsAttention` (Astra F4) is `ListRuns`' own per-run attention computation, reusing sqlite's `mailboxStatuses` (the same function `LoadRunStatus`'s `Mailboxes` population calls) so the bare listing and the `-run` detail can never disagree |
| [submission.go](submission.go) | `SubmissionStore`, `LaunchClaim`, `LaunchClaimState`, `LaunchClaimSettlement`, `LaunchClaimRepository`, `ResultSubmission`, `SubmissionOutcome`, `SubmissionOutcomeKind`, `TransientReason` (`TransientAttemptNotRunning`, `TransientUndeliveredMessages`), `TransientReasonOf`, `ClaimedSubmission`, `ClaimedSubmissionFieldLimit` | Worker-authority writes with no controller lease: launch/check-exec claims, `SubmitResult`'s atomic section 7 handoff, `RecordMalformed` for a submission that failed parsing before any typed identity existed, monotonic stop requests. `LaunchClaim.SessionID` (required; `AttemptID` now optional, empty for an attempt-less session) is the Phase 3 session-keyed claim addition; `LaunchClaim.SeedEvidence` records the workspace-trust pre-seeding outcome (evidence only — no decision ever reads it). The typed transient reason a retryable result or verdict carries (`SubmissionOutcome.Transient`), classified from the acceptance error by `TransientReasonOf` in the store and the fakes alike. |
| [runtime.go](runtime.go) | `Runtime`, `ErrPaneNotFound`, `WorktreeRequest`/`WorktreeInfo`, `WorkerPaneRequest`, `PaneHandle`/`PaneRef`, `ProcessInfo`, `PaneProcess`, `WorkspaceRuntime`, `WorkspaceRequest`/`WorkspaceHandle`/`WorkspaceRef` | Herdr's pane and worktree surface: `CreateWorktree`, `OpenWorkerPane` (a `layout.apply` command pane), `FindPaneByLabel` recovery, `ReadPane` evidence, `InspectPane` occupant identity (reporting the typed `ErrPaneNotFound` for a pane that positively does not exist — distinct from any inspection failure — and stamping `PaneProcess.ServerInstance` with the server lifetime that answered that very request, "" when unknown), `ClosePane` (a pane id the server does not have is `ErrPaneNotFound`: nothing was closed), and `ServerInstance` — the opaque identity of ONE server lifetime behind the configured socket (the adapter's peer pid plus that process's start time, so a pid reused by a later server never repeats it; "" when unknown, and a token of an older format never equals a current one), compared by equality only. `SendText` (the fixed-grammar launch-line fallback transport) was deleted in slice 6: HOP delivers everything by pull and never had a call site for it. `WorkspaceRuntime` (manager placement, S8-pinned shapes) is a SEPARATE interface from `Runtime`, consumed through the optional `Controller.Workspaces` field, for the same reason `WorkflowRepositories` is separate from `UnitOfWork` |
...
| [grammar.go](grammar.go) | `GrammarVerb*`, `Grammar*Line` constants and renderers, `GrammarReason*`, `GrammarRefusalLine`, `GrammarSubmissionTransientLine`, `GrammarResultRefusalLine` (hop result submit's `<kind>: <detail>` first line for stale, conflicting and malformed), `GrammarPrincipal`, `GrammarTime` | The section 7 worker-protocol grammar's one source of truth: verb spellings, every worker-facing first line (the retryable `transient:` lines behind `GrammarTransientPrefix` included), the fixed follow-on lines, the rendering conventions and the enumerated refusal reason tokens behind `refused: <token>`. Templates, cmd/hop and the fixtures QUOTE these; `TestGoldenGrammar` pins every constant and rendered line against retyped golden literals, and slice 6's real-binary contract tests assert the same constants per verb. STATUS-1 extends the same file with `hop status -run`'s own small grammar (section 10): `GrammarAttentionLine`/`GrammarAttentionActionSession`/`GrammarAttentionActionHuman`/`GrammarAttentionMarker` (section 7's mailbox condition, verbatim), `GrammarTaskLabel`/`GrammarTaskLine`/`GrammarTaskAddress`, `GrammarIntegrationLine`, `GrammarShortfallLine`/`GrammarShortfallVerdictRejected` (kept as its own constant rather than an `internal/domain/run` import, parity pinned by `TestGoldenGrammar` against `run.ShortfallVerdictRejected`), `GrammarShortfallEvidenceInconsistent` (the token `ShortfallEvidenceInconsistent` is defined from, rendered through `GrammarShortfallLine` with no task), `GrammarVerdictRejectedLine` (Astra F3: the one shortfall shape that names a review — id, subject commit, then its already-safe-rendered reasons path last, since a path may contain spaces), `GrammarQuestionLine`/`GrammarAnswerInvocationLine`, `GrammarSessionLine`, `GrammarSessionLaunchCorroborationAction` (the value-free action a session reconciling because another process on its pane carries the launch identity renders under its session line), `GrammarSessionRestartNote` (the value-free note under a session line saying what the restart-lifecycle step did to it — closed, or closed and relaunched — rendered ONLY from `RestartDispositionFor`'s two fixed values, so no journal reason, lifetime token, pid, label or path can reach that surface; a manager's note lands on the manager's own session line, which is what makes a manager relaunch separately visible from a worker's) and `GrammarActionPrefix` (the literal `action:` token every nested human-action line begins with, Astra F2's documentation tidy) — cmd/hop resolves every task/session uuid to its `t<seq>` label before calling these, so the constants themselves take already-resolved strings, exactly like `GrammarMessageLine` above takes an already-resolved `from`; paths and Herdr binding identifiers are cmd/hop's `safeRenderExternal`'s job, not a grammar constant, since escaping is a rendering DECISION, not fixed text. `GrammarSubmissionTransientLine` is the one mapping from a typed transient reason to the submit verbs' retry line (unknown reason: no line). NOTICE-1 adds the controller notice grammar: `GrammarNoticeTaskLine` (the task-consequence line, ALWAYS first in both notice shapes), `GrammarNoticeIntegrationLine` (an integration settlement's own line, second), `GrammarNoticeReasonLine`/`GrammarNoticeEvidenceLine` and `GrammarNoticeObligationsLine`/`GrammarNoticeObligationsNoneLine` — used by `renderTaskNotice` (usecase_featurecheck.go) and `renderIntegrationNotice` (usecase_featuresettle.go), design section 7's "the controller notice grammar". Here alone, escaping IS a grammar concern rather than only a rendering one: `GrammarNoticeReasonLine`/`GrammarNoticeEvidenceLine` render their externally sourced field (a check or git command's own detail text, a filesystem path) through `RenderExternal` themselves, since line POSITION is this grammar's parseable meaning and an unescaped newline in either field would forge an extra line (a fake task-consequence line, ESC-1's threat one layer further out) — every other field on these lines (taskSeq, the integration uuid, the typed consequence/state, the obligation uuids) cannot carry one. |
| [render.go](render.go) | `RenderExternal` | The one rendering boundary for an externally sourced string shown as a single field: raw when it is valid UTF-8 with no C0/DEL/C1 control byte, no double quote and no backslash, else `strconv.Quote`'s form. cmd/hop's `safeRenderExternal` delegates to it byte for byte; the launch-ended and unplaced-launch details render a durable creation label through it |
| [templates.go](templates.go) | `renderManagerAssignment`, `renderWorkerProtocolCrib`, `renderTaskAssignment`, `renderReviewAssignment`, `priorAttemptFeedback`, `FreezeWorkflowArtifacts`, `WorkflowFreezeRequest`, `IntegrationBranchName`, path derivations (`roleArtifactPath`, `reviewReasonsPath`, `checkEvidencePaths`) | The section 6 assignment/role/review artifacts: pure, deterministic, frozen-run-facts-only templates quoting every protocol line from grammar.go (`TestTemplatesQuoteGrammar`); the implementer and reviewer assignments state the worker's exits — an accepted or duplicate submission means the work is done and the turn ends without polling, a `stale: <detail>` or `refused: stale` first line (and, for a reviewer, `refused: not-reviewer`) means stop without retrying, and a `refused: subject-mismatch` reviewer resubmits with the frozen subject; a reviewer's `refused: malformed` means fix the named problem and resubmit, and `refused: conflicting` means stop; the crib's preamble gives the `refused: <reason-token>` shape and names the verbs whose sections give their own (`hop result submit`'s `<kind>: <detail>` lines, and a refused fetch, which prints no first line); `FreezeWorkflowArtifacts` is slice 6's feature-freeze hook — role files read through `ArtifactStore`, copied into the run's artifact directory with digests, the crib written beside them, the integration branch name derived from the run sequence (never a config key). STATUS-1 adds `managerAssignmentFields.RepositoryRoot` (frozen at both call sites — `StartFeatureRun`'s `req.RepositoryRoot` and `ensureManagerArtifacts`'s `frozen.RepositoryRoot`, so a recreated assignment still reproduces its frozen digest) and `renderManagerAssignment`'s verdict-channel paragraph: run `<hop> status -C <repository root> -run <run uuid>` after any controller info notice and read its shortfall lines, since the review-verdict notice's own body is only the reasons artifact (section 7's "the manager's verdict channel"). `posixShellQuote` (Astra F1) POSIX-single-quotes each of the three concrete arguments (hop path, repository root, run id) independently before they are interpolated into that one shell command line — the only place this file builds a command from frozen facts rather than literal placeholders; every OTHER verb line stays unquoted on purpose (pinned prompt goldens 7a parses). The paragraph's decision rule (Astra F3): plan exactly one fix task ONLY when a `verdict-rejected` shortfall's `reasons=` path equals the fetched notice's own body path — that identifies the notice as THIS review's rejection, not a stale or already-addressed one; otherwise act on the notice itself (a needs-rework notice keeps its own retry path). A shortfall naming `evidence-inconsistent` is stated to be never a rejection: plan no fix from it, act on the notice, and ask the human to inspect the run |
...
| [controller.go](controller.go) | `Controller`, `RunHandle`, `Heartbeat`, `Detach`, `ErrStopRequested`, `claimIntegrationStep` | The driving service composition calls; ports as fields, plus `GitExecutable` (the absolute path of the git binary every repository/worktree command runs, resolved once by composition). `RunHandle` is an opaque per-run token (run identity, the held lease and a dispatch scope) so composition never touches identity types. The dispatch scope also carries the integration step's non-blocking exclusion (`claimIntegrationStep`), held for the whole of one `DriveIntegration` or `DriveIntegrationCheck` call. `Heartbeat` extends the lease on the design's interval and cancels in-flight external calls on failure, except a failure caused only by the caller's own ended context, which says nothing about the lease and leaves the dispatch scope live; `Detach` journals, cancels and releases without stopping; `revalidateForDispatch` is the section 4 step 2 revalidation every external mutation runs first. `Messages`/`Plan`/`Reviews`/`Workspaces`/`Presentation` are optional Phase 3 fields, nil for a solo-only Controller (`Presentation` is the Phase 1 `AgentPresentation` port, consumed by section 9's run-state token publication) |
...
| [usecase_unplacedlaunch.go](usecase_unplacedlaunch.go) | `resolveUnplacedLaunch`, `unplacedLaunch`, `unplacedLaunchFacts`, `readUnplacedLaunchLocked` | Phase 2's unbound-launch rule keyed to one session, used by feature stop and per-attempt/completion/terminal-failure retirement for a session with no committed binding: the session's unresolved launch journal read in one transaction (`readUnplacedLaunchLocked`: its unresolved intents, the newest one's own claim, unusable rows that might name it, whether it is bound); a claimed launch whose creation label answers nothing goes through the label-only launch-ended row and, once settled, has nothing live; a live or unobservable claimed process stays outstanding with the reason named |
| [usecase_launch.go](usecase_launch.go) | `CorroborateLaunch`, `LaunchProgress`, `launchClaimedLocked` | One inspection round toward settling a launch claim under the section 6 predicate: settled, needs-interaction (forking wrapper), failed (`exec_failed`, terminating the session), already-settled (early acceptance; activates a still-launching session) or still pending — recovers a lost binding by creation label, never sleeps or resends. The launch-claim deadline's own transaction re-reads the claim (`launchClaimedLocked`, shared with the session-keyed sibling): the bound applies only while no claim exists, so a launcher that claimed after the round's earlier read keeps its pending intent — the session's pre-binding principal authority — instead of being refused by the round meant to bound its silence |
| [usecase_stop.go](usecase_stop.go) | `RequestStop`, `DriveStop`, `StopReport`, `CheckRunIntent`, `closePaneOperation`, `closeTargetProcessStillLive` | One stop round: retire every check execution's group under the group-retirement rule and the recorded worker under the shared pane.close operation procedure, and mark stopped ONLY once all owned work is observed absent; mismatches and failed inspections stay outstanding, never absent. A close target flagged `requireProcessGone` (an unsettled launch claim) retires only with its claimed process also observed gone; a `ClosePane` answered not found closed nothing and the re-observation decides |
| [usecase_restartlifetime.go](usecase_restartlifetime.go) | `ReconcileServerRestart`, `RestartReport`, `RestartOptions`, `restartIdentity`, `restartDisposition`, `identifyRestartedPane`, `restartRecordedPID`, `findRestartClose`, `openRestartClose`, `dispatchRestartClose`, `finishRestartClose`, `redriveRestartClose`, `restartCloseTarget`, `restartCloseOutstanding`, `restartDispositionFor`, `restartRetirementFacts`, `readRestartRetirementFacts`, `restartAttemptSettledLocked`, `applyRestartDisposition`, `restartInterruption`, `settleRestartLaunchEnded`, `restartCandidateSessions`, `restartSessionReason`, `restartRelaunchReason`, `launchEndedRestartReason`, `RestartDispositionFor`, `RestartDispositionClosed`, `RestartDispositionRelaunched` | The restart-lifecycle step (docs/plan/phase-3-design.md sections 4 and 6): on a CHANGED server lifetime — `ClassifyServerLifetime`'s positive verdict, read BEFORE and AFTER every observation and required to agree with itself, so observations spanning a restart conclude nothing — HOP closes the panes it owns and cold-relaunches those sessions from their recorded native references. CHANGED replaces the CONTINUITY conjunct ONLY: the recorded process (the claim's pid, else the corroborated occupant's) must still be observed gone by a successful group listing, exactly as before. The close never goes by recorded pane id alone, because a pane id is not a durable address across a restart — a workspace closed before one leaves its id free, the restarted server reissues it, and the composed pane id answers for somebody else's pane (`TestSpikeRecordedPaneIDCanAddressADifferentPane` observed that collision). `identifyRestartedPane` is the ladder: the creation label resolving to exactly the recorded id (a layout attribute, so it speaks in BOTH restore windows), else exactly one foreground member running this session's own native restore invocation (`MatchRestoredHarness`, which survives a human's rename); anything else — a label answering another pane, a failed lookup, an unclaimable occupant, two restore members, or neither address answering — closes nothing and reports a value-free reason. `MatchRestoredHarness`'s None outcome is not suspicious and never fail-closed on its own: it is the ordinary answer for a pane whose restore has not fired. The close journals its own `pane.close` intent under `closeReasonRestart`, freezing the OBSERVING lifetime (the placement's is gone), so the absence confirmation brackets against the lifetime that serves the socket now; `findOrCreateCloseOperation` defers to it rather than adopting one. An unresolved close is RE-DRIVEN from its persisted target and re-ACTED (`redriveRestartClose`) while the pane is not yet observed absent, never re-identified: only the act is repeatable, and an act that failed — or a lease fenced between the intent and the act — otherwise leaves the pane, and the agent Herdr restored into it, standing under an intent that says a close is under way. The re-act is bounded by the lifetime the intent froze, which is load-bearing rather than defensive: a pane id is a durable address only WITHIN one lifetime, and `observePlacedPaneAbsence` does not bound it either, since its continuity conjunct guards only the branch where the pane is observed absent. A close that lands journals its dispatch (`recordCloseDispatched`), the one record that tells a later round — and `restartCloseOutstanding`'s human-facing line — a close carried out and awaiting its observed absence from one whose act never landed. `restartDispositionFor`'s five branches, in design section 6's order: a held stop, a durable terminal-failure cause (`featureFailureCauseLocked`, the same cause that suppresses every other launch — a relaunch is a launch) or a solo run, RETIRE (the close IS the retirement; without it the run stays `stopping` forever and `observeWorkerExit` never fires); a session whose attempt has already reached a retirement boundary (`restartAttemptSettledLocked`: the three `retirementCandidates` reads — a terminal attempt outcome, an accepted result, an accepted verdict) RETIRES, being due for retirement anyway; an unsettled claim settles `exec_failed` under `launchEndedRestartReason`, a third recognized launch-ended reason, because a pre-assigned native reference names no transcript for a harness that never started (`TestSpikeClaudePreassignedSessionID`); a settled claim with a Claude harness and a native reference is COLD RELAUNCHED through the existing `coldRelaunchFeatureSession` (never a second path), with the manager reconciled LAST so a round that fails partway has not moved the run's lineage; anything else is interrupted and the manager decides. Solo runs take this step only under a held stop, since solo cold relaunch stays exclusively a `hop resume` action. `RestartDispositionFor` is the ONE mapping from this rule's journaled reasons to what `hop status` renders: the read model reduces a session's newest transition reason through it by PREFIX, relaunch first (the relaunch reason extends the close reason), so EVERY branch that closes a pane renders one of the two fixed value-free strings — the interrupt's reason and the settled launch's own launch-ended reason included — and the journal keeps the detail |
| [usecase_launchended.go](usecase_launchended.go) | `observeLaunchEnded`, `settleIfLaunchEnded`, `observeClaimedProcessGone`, `serverContinuityHolds`, `placedContinuityDetail`, `observePlacedPaneAbsence`, `launchEndedReason`, `launchEndedByController`, `claimedProcessLiveDetail`, `observeUnplacedLaunchEnded`, `settleIfUnplacedLaunchEnded`, `launchEndedUnplacedReason`, `unplacedLaunchEndedOutcome`, `launchEndedResolution`, `launchEndedIntentLocked`, `unplacedLaunchLiveDetail`, `unplacedContinuityDetail`, `placedUnrecordedLifetimeDetail`, `placedAbsenceDetail`, `unplacedUnrecordedLifetimeDetail`, `unplacedAbsenceDetail` | The launch-ended decision row (docs/plan/phase-3-design.md section 4): an `exec_pending` claim whose own current, unsuperseded binding's pane is positively absent (`observePaneAbsence`) AND whose claimed process is gone from its own process group, both observed between two `serverContinuityHolds` reads naming the binding's recorded server lifetime, settles `exec_failed` under the lease with the fixed reason and the pane and pid as evidence; everything short of that set is ambiguous. Its label-only variant, for a launch whose pane.open outcome (and so binding) was never recorded: no binding, exactly one unresolved decodable labeled pane.open naming the session (nothing unusable that might), that intent's own `exec_pending` claim, a successful creation-label lookup finding nothing and the claimed process gone — bracketed by continuity reads naming the intent's recorded `server_instance` — settle the claim `exec_failed` with the distinct `launchEndedUnplacedReason` (pid as evidence) AND the pane.open `failed` with `unplacedLaunchEndedOutcome` (dispatched, never refused), in one transaction that re-reads the facts; `launchEndedIntentLocked` lets `sessionLaunchClaimLocked` keep reading that claim through the resolved intent. The continuity conjunct exists because a renamed pane is restored under its new name after a restart and a pane awaiting a deferred native restore answers `pane_not_found`; without it each row reports the value-free rename-back action (`placedContinuityDetail`); a launch whose pane is really gone after a restart has no exit yet. `observePlacedPaneAbsence` is the same bracket applied to any placed pane: stop's claimless worker, the pane.close procedure (both observations, against the lifetime frozen into its intent), feature stop's and the retirement's claimless sessions, and the live self-exit observation (`observeWorkerExit`) conclude absence only through it, reporting `placedContinuityDetail` otherwise. Every label a detail shows renders through `RenderExternal`. Corroboration, resume, stop and the terminal-failure shutdown all decide by both Continuity can fail two ways, told apart only in what the human is told (`placedAbsenceDetail`/`unplacedAbsenceDetail` pick between them): a lifetime that WAS recorded and no longer matches is the restart case, whose action is to rename a renamed pane back; a placement that recorded NO lifetime can never establish continuity for any observation at any later time, so absence is never concluded for it — nothing was renamed there, so it carries its own action instead of the rename-back one. `Runtime.ServerInstance` is implemented on darwin alone, so on every other platform every placement has that second shape (design section 6's second residual). |
| [usecase_sessioncorroborate.go](usecase_sessioncorroborate.go) | `CorroborateSessionLaunches`, `SessionLaunchProgress`, `sessionUnderLaunchCorroboration`, `wrapperReconciliation`, `sessionReconcileLaunchCorroboration`, `settleSessionExeced`, `settleSessionExecFailure`, `applyManagerRunRunning`, `driveLaunchingRunFailure`, `recoverSessionBindingByLabel`, `driveSessionLaunchDeadline` | The feature-mode, per-session sibling of `CorroborateLaunch`: one inspection round for every launching session under the one corroboration predicate, PLUS the one reconciliation this step itself produces — the live wrapper reconciliation, identified structurally by `wrapperReconciliation` (a current unsuperseded placed binding whose incarnation's claim is still `exec_pending`), so a later round settles it once one observation is clean while a matching process still there writes nothing; the live fail-closed branch records its own value-free reason (`sessionReconcileLaunchCorroboration`), never resume's; the manager's settlement moves a launching run to running (never a resuming one — `ResumeFeature`'s gate). An exec_failed claim is terminal, placed or not: a child's settles through `settleChildExecFailure`; a manager's session is terminated and, while the run is launching (the pass runs nothing else then), `driveLaunchingRunFailure` fails the run through `RetireSettledSessions` on this and every later round until it settles. A placed `exec_pending` claim whose pane inspection answers not found goes through `settleIfLaunchEnded`: settled `exec_failed` only on the corroborated-absence pair, then the same exec_failed consequences (`settleSessionExecFailure`); pending otherwise. An unbound `exec_pending` claim is first recovered by its creation label and, when no pane answers, goes through the label-only variant (`settleIfUnplacedLaunchEnded`) with the same consequences |
...
| [usecase_submit.go](usecase_submit.go) | `SubmitResult`, `SubmitResultRequest`, `SubmitResultResult`, `ErrTransientReasonInvalid`, `checkTransientReason` | Section 7 step 1 (parse and bound the inputs) and the canonical digest, computed here; steps 2-5 are `SubmissionStore.SubmitResult`'s contract. The store outcome's typed transient reason is carried to the CLI as `SubmitResultResult.TransientReason`; a reason that does not match the outcome kind (a transient outcome with no known reason, or a reason on any other outcome) is `ErrTransientReasonInvalid`, never a guessed retry line |
...
| [usecase_integration.go](usecase_integration.go) | `DriveIntegration`, `IntegrationReport` (`CheckDue`, `InFlight`), `ResolveIntegrationHead`, `OpIntegrationMerge`/`Publish`/`Reset`/`Fence`, the `integration*Intent`/evidence payloads, `integrationMergeArgv`, `releaseEligibleTasks`, `integrateLocked`, `prepareIntegratedNotice` | The section 4/8 serial-integration driver (slice 2b): claim the lowest-seq completed implement task into the store-enforced serial slot (pre-merge head from the live ref, never rows); `integration.merge` as one execution per operation — materialize to OBSERVED completion with the HEAD verified and recorded as act evidence before the spawn, the frozen noninteractive argv (Q5(a): repo-local hooks suppressed via `-c core.hooksPath=<empty dir under the operation's artifact dir>`, created before the spawn), classification merged/no-op/conflict/failed; `integration.publish` update-ref CAS with the three fencing layers; a passing combined candidate integrates the task, releases dependents and commits the manager notice in ONE transaction (`integrateLocked`). A checking integration with no settled receipt is reported `CheckDue` and never executed here: the combined check is `DriveIntegrationCheck`'s, which a caller runs under a context it can cancel. A call made while that round holds the step does nothing and reports `InFlight` |
| [usecase_integration_check.go](usecase_integration_check.go) | `DriveIntegrationCheck`, `isIntegrationStepOperation`, `unresolvedIntegrationStepOperation`, `runIntegrationCheck`, `runStopRequested`, `combinedCheckSettlement`, `settleCombinedCheck`, `settleUnspawnedCheck`, `failIntegrationCheck`, `failCheckLocked` | The combined-candidate check execution: the generalized pipeline's integration subject (`integrationCheckIntent` on an `OpCheckRun` row until migration 002's typed request table), its own detached checkout under the check operation's directory, mandatory evidence retention, and the integration/feature outcome dispatch — a passing check landing after a stop records evidence and never integrates. `DriveIntegrationCheck` is the integration step's cancelable round (design section 8). It acts only on an integration still in checking with no unresolved integration-step operation (`isIntegrationStepOperation`), and it shares `DriveIntegration`'s exclusion. The intent refuses under a held stop or a terminal-failure cause (`featureFailureCauseLocked`), reporting `Interrupted`. A round that ends before its spawn (a refused revalidation, or a canceled materialization) settles the execution failed as never spawned (`settleUnspawnedCheck`) under a context that outlives the cancellation. `settleCombinedCheck` commits the outcome, its evidence and the integration's consequence in one transaction, and writes nothing once the execution is already settled — a terminal-failure shutdown that retired it owns the integration The integrated notice's BODY is written only after a read-only look at the run's held-stop flag (`runStopRequested`): the settling transaction's own read stays the authority for the decision, and this one only keeps a check that passes under a held stop — whose notice the stop path discards — from leaving a body under the run's messages directory that no message row names. |
...
| [usecase_featurecheck.go](usecase_featurecheck.go) | `DriveFeatureChecks`, `FeatureCheckReport`, `settleFeatureCheckOutcome`, `recoverFeatureCheckExecutions`, `requeueUnspawnedFeatureCheck` | The per-task (result-subject) check pipeline at feature scope: the `result, feature` outcome row — pass completes attempt and task with the RUN staying running; fail applies the budgeted task consequence; the unknown-outcome rule requeues (repeatable) or settles by budget; stop precedence interrupts inside the outcome transaction. A round that ends before its spawn (a refused revalidation, or a cancellation during the candidate inspection or the materialization) settles the execution failed as never spawned and returns its request to requested (`requeueUnspawnedFeatureCheck`); the attempt and task are untouched. Every settlement runs under a context that outlives the round's cancellation |
...
| [usecase_featureresume.go](usecase_featureresume.go) | `ResumeFeature`, `ResumeFeatureRequest`/`Result`, `FeatureSessionReport`, session dispositions, `coldRelaunchFeatureSession`, `reconcileFeatureSession`, `markSessionReconciling`, `placedLaunchInFlight`, `placedLaunchProcessObserved` | Feature-mode resume: takeover by lease CAS, a held stop routed to stop handling before any adoption, integration operations recovered first, then the feature startup continuation (`continueFeatureBootstrap`: while the manager is reserved, `integration.init` re-driven or recovered and the manager placed and launched; a launching manager recovered by label with the run returned to launching, its pending report not holding the run reconciling), then the per-attempt launch recovery (`resumeAttemptLaunches`: continue mode, resolve-only while a terminal-failure cause stands; a child whose pane it opened reports pending without holding the run reconciling, and an unresolved one's report names its condition), EVERY session reconciled (while a terminal-failure cause stands, an unplaced launch whose worktree work is resolved reports `SessionLaunchSuppressed` and does not hold the run reconciling: the run returns to running and the controller loop's retirement pass — which reads the cause there — retires it and fails the run) under the one corroboration predicate; cold relaunch is authorized per session only through `--confirm-absent <session-id>` — the attestation journaled (`absence.attested`) with both assertions and the recorded/observed server-instance evidence, relaunch requiring positive absence AND `ServerContinuityEstablished`, Claude-only; a manager relaunch creates the SUCCESSOR manager session bound to the same native reference with the predecessor marked terminal in the same transaction, children keeping their historical `parent_session_id`. An exec_failed claim is retired with no process: a child's settles through `settleChildExecFailure` exactly as launch corroboration settles it (a controller that died before corroborating never strands the attempt); the manager's session is only terminated, and the manager-lineage failure cause fails the run on the next retirement pass. A placed `exec_pending` claim first goes through the launch-ended row (`settleIfLaunchEnded`), so a launch that ended before corroboration is retired the same way (its report detail names the reason) instead of holding the run reconciling; a vanished pane whose claimed process still runs reports pending with that detail. An unbound session is first adopted by its creation label (the decision table's takeover rule), and an unbound `exec_pending` claim then goes through the label-only variant (`settleIfUnplacedLaunchEnded`). A child's placed launch left unsettled is in flight (`placedLaunchInFlight`) — launching, binding current, claim absent or that placement's own `exec_pending`, its pane.open not failed, the recorded pane answering by id with that inspection's own `ServerInstance` stamp equal to the binding's (so a restart between an earlier identity read and the inspection cannot pass), and a foreground occupant whose own process is the launch's: the claimed process, or, with no claim yet, this session's own launcher (`pinnedLauncherOccupies`: the member at the pane's shell pid reporting argv exactly equal to the placement's frozen `[<abs hop> launch --run <run> --session <session>]` command) — and does not hold the run reconciling: the run resumes and the loop's corroboration finishes it; anything weaker (a changed, unknown or older-format lifetime, whose public pane ids survive a restart; a pane gone by id; another process; any occupant but this session's launcher before a claim; a reconciling session that is not the launch corroboration's own revisitable reconciliation) keeps the run resuming with the reason named. `markSessionReconciling` takes its transition reason explicitly, and its doc comment carries the invariant the structural identification rests on: every resume path reaches it only after reading that session's claim as SETTLED, so a reconciling session with an `exec_pending` claim under a live placement can only be the live corroboration's own. `placedLaunchProcessObserved` is the positive-observation half of the in-flight rule (the pane answering by id under the placement's own lifetime stamp, with its own process being the claimed one, or this session's launcher before a claim), shared verbatim with the manager bootstrap's continuation of the same two states |
...
- The run controller follows the section 4 transaction rule throughout:
  ...
  dispatched, and a check execution refused (or canceled) before its
  spawn, which settles failed as never spawned — and unstarted work is never
  ...
  check command included) are canceled with the lost lease; a heartbeat
  refused only because its caller's context ended leaves the scope live,
  so an interrupted round's revalidation never cancels the acts after it.
...
- Every close runs the shared pane.close operation procedure
  ...
  and the server lifetime its absence is decided against: the
  placement's, or for a positive-evidence retirement the lifetime that
  identified the occupant; a close journaled without one takes its
  caller's) before any act; pane scrollback is captured through the `ArtifactStore`
  ...
  group are observed absent. For a target whose launch claim is still
  `exec_pending`, the feature stop and the per-attempt and terminal-failure
  retirement require the claimed process observed gone too
  (`requireProcessGone`, from the claim's current state, never persisted):
  a vanished pane whose claimed process still runs stays outstanding —
  the operation reconciling — with the human action named. A `ClosePane`
  answered `ErrPaneNotFound` closed nothing: no dispatch is recorded and
  the immediate re-observation decides.
- Pane absence is one strict rule (`observePaneAbsence`): the pane must be
  ...
  id or by label is never absence. For a placed session outside resume
  (stop, the close procedure, retirement, the self-exit observation) the
  rule is bracketed by server continuity (`observePlacedPaneAbsence`):
  absence observed without the recorded lifetime serving the socket on
  both sides is ambiguous, since a renamed pane restored after a restart,
  or one awaiting a deferred native restore, answers neither lookup.
- The launch-ended row (`settleIfLaunchEnded`): an `exec_pending` claim
  whose own current, unsuperseded binding's pane is absent under that rule
  AND whose claimed process is gone — a successful
  `ProcessGroupInspector.GroupProcesses(claim pid)` listing with no member
  carrying the pid (the launcher is a session leader, so a living claimed
  process is always listed in its own group; pinned by
  test/integration's `TestSpikeVanishedPaneShapes`) — settles
  `exec_failed` with `launchEndedReason` and the pane and pid as evidence,
  inside a transaction that re-reads the claim and the binding. The
  durable claim state then drives the ordinary exec_failed row
  everywhere, so a crash after the settlement is finished by any later
  round or by resume. A missing inspector, an uninspectable pid, a failed
...
- `hop resume --confirm-absent` journals an `absence.attested` operation
  ...
  equal; under the lifetime-scoped `ServerInstance` contract token
  equality is the whole predicate — with the pane already positively absent by id
...
- The Phase 2 run-controller scenario suite runs against `fakeStore` (one
  ...
  (`sessionIncarnationCurrentLocked`: the current binding, ANY disagreeing
  or unusable pending intent of the session failing closed
  (`pendingSessionIntentDisagreesLocked`, read as generic JSON like the
  store's `json_type` check), else the session's newest pending intent
  ...
  `fakeStore.Heartbeat` refuses a canceled context, as the real
  heartbeat's write transaction does, and
  `TestHeartbeatUnderCallerCancellation` pins that a caller's canceled
  context leaves the handle's dispatch scope live while a lost lease
  ends it (`DispatchLiveForTest`). The other fakes honor a canceled
  context the same way: `fakeStore.Begin` refuses one (the real
  `BeginTx`), a unit of work begun under a context canceled before
  `Commit` fails the commit (database/sql's `Tx`), and
  `fakeCommands.Run` starts nothing under one (the real Runner's
  refusal). `fakeGitRepo`'s `CheckExecGate` and `GitGate` hold a
  simulated command open under its act context, the way the real Runner
  holds a child, and `canceledCommand` answers one the way the Runner
  answers a canceled command.
  ...
  adapter's `TestRunnerReportsTruncation` runs against the real Runner.
  `fakeRuntime.InspectPane` stamps every observation's `ServerInstance`
  as the adapter does — `ServerInstanceValue` read before and after the
  scripted inspection, stamped only when both agree (unknown while
  `ServerInstanceErr` is scripted) — and scripted identities use the
  adapter's pinned token format (`fakeServerToken`). Scripted `InspectPaneFn`
  ...
  transport. A vanished pane answers with `pinnedPaneNotFound`
  (fakes_ports_test.go), the Herdr adapter's error shape for
  pane.process_info, pane.report_metadata and pane.close that
  `TestSpikeVanishedPaneShapes` (test/integration/spike_panevanish_test.go)
  pins; `runStatePresentation.Vanished` and `fakeRuntime.ClosePaneErr`
  script it for presentation and close.
  `fakeGroups` refuses a group id of 1 or less as the process adapter
  does, reads an unscripted group as empty (a successful listing with no
  member left), records every listed id, and `liveLeader` scripts the
  pinned live relation — a pane's process leading its own group. A
  same-state claim resettlement is a no-op, as in the real store, and a
  settlement to anything but `execed` or `exec_failed` fails the commit.
...
- `go test ./internal/app -run 'TestGoldenGrammar|TestGoldenRolePrompts|TestWorkerPromptsFollowTheTransientLine|TestTemplatesQuoteGrammar|TestCribRetryableLinesPerVerb|TestCribRefusalShapes|TestFreezeWorkflowArtifacts'` —
  ...
  under exactly the result and review submit verbs), the crib's refusal
  shapes (the scoped preamble, result submit's own refusal line, the
  no-line refused fetch, and no other section claiming a shape), and the
...
- Phase 3 scenario suite, against the same `fakeStore` extended with
  ...
  - `go test ./internal/app -run 'TestCreateTask|TestClosePlanReopenOnCreate|TestClosePlanRefusesEmptyPlan|TestRequestRetry|TestPlanVerbsTransientUntilRunning|TestPlanRefusalReasonsAlwaysSet'` —
    ...
    inside `RequestRetry` itself); while the run is launching, resuming
    or completing every plan verb is transient with nothing changed, and
    the same request IDs once the run is running are accepted, then
    duplicate; a completed run, or a resuming one with a stop request,
    refuses `run-not-accepting`.
  - `go test ./internal/app -run 'TestMessagingReferenceTraceRelayedQuestion|TestMessageVerbsAcrossRunNotRunning|TestSendMessageValidation|TestAckMessageRequiresOwnDelivery|TestAckMessageRequiresCurrentIncarnation|TestShowMessage|TestFetchMessageEmptyQueueCommitsNothing|TestFetchMessageCrossRunRefused|TestAckMessageCrossRunRefused'` —
    ...
    idempotency); while the run is resuming, a relay is transient and
    enqueues nothing while fetch, ack and an answer proceed, and the
    same relay once running is accepted, then duplicate; send addressing/kind legality, mailbox-closed refusal,
  ...
  - `go test ./internal/app -run TestFakeAnswer` — `fakeStore`'s half of
    the answer authority and admission scenarios
    (`fakes_answer_test.go`, mirroring the sqlite adapter's
    `answer_admission_test.go` request for request): a worker or the
    manager answering a human question, another task's question or its
    own question is refused `unauthorized` with nothing inserted and a
    human question left unacknowledged, while the recipient's answer and
    the human's `hop answer` are accepted; the retry precedence matrix
    (the sender's derived address, then the request-ID receipt, then
    authority, then prior content) and a session claiming the recipient's
    address refused `unauthorized` whatever it replays
    (`TestFakeAnswerReplayByMisclaimedAddress`); an answer
    before result acceptance forcing the transient drain, and one after it
  ...
  - `go test ./internal/app -run 'TestFakeSubmitResultTransientReasons|TestFakeSubmitReviewTransientReasons|TestSubmissionTransientReasonContract'` —
    ...
    half): an unsettled launch claim is attempt-not-running and a pending
    mailbox undelivered-messages for results and verdicts alike, a
    launching review attempt with a pending mailbox is attempt-not-running
  ...
  - `go test ./internal/app -run 'TestFakeSubmitReviewRefusalReasons|TestFakeSubmitReviewOrder|TestReviewRefusalReasonContract'` —
    `fakes_review_refusal_test.go`, the fake half of the typed review
    refusal reasons (the sqlite adapter's `review_refusal_test.go` is the
    real half): another candidate is `subject-mismatch`, the manager's
    session `not-reviewer`, a replaced incarnation `stale`, accepted and
    duplicate carry no reason and a different verdict is `conflicting`;
    the featureStore wrapper decides in the real store's order
    (existence and agreement are `malformed` even for a caller that is
    not the reviewer, and a prior verdict answers before the reviewer
    check) and closes the mailbox inside the acceptance's own critical
    section; and `SubmitReviewVerdict` returns `ErrReviewReasonInvalid`
    for a store outcome whose reason its kind does not admit.
  ...
  - `go test ./internal/app -run 'TestFakeFetchRequiresTheTaskCurrentAttemptSession|TestFakeAckRequiresTheTaskCurrentAttemptSession|TestFakeManagerMessagingRequiresAManagerSessionThatHasNotEnded|TestFakeSendRequiresTheAddressCurrentSession'` —
    `fakes_message_currency_test.go`, the fake half of the sqlite
    adapter's `messaging_currency_test.go`, shape for shape: the fake's
    send (every kind, after the receipt and the incarnation), fetch and
    ack decide the address's current session through
    `addressSessionCurrentLocked` (`run.CurrentAddressSession` over the
  ...
  - `go test ./internal/app -run TestStatus` — the presentation/status
    ...
    head, every subject's tree id distinct from its commit id — a real
    `SubmitReviewVerdict` reject of the head correlating to its fetched
    notice and turning stale once a fix moves the head
    (`TestStatusVerdictRejectedShortfallCorrelation`), a real approve of
    the head leaving no verdict shortfall
    (`TestStatusApproveOfCurrentHeadClearsTheVerdictGuard`), and
    disagreeing recorded trees rendering only plan-open and
    `evidence-inconsistent` beside an otherwise ordinary block
    (`TestStatusEvidenceInconsistentHead`); `TestStatusGuardShortfallsHeadTreeRule`
    pins `StatusGuardShortfalls` itself — `check-missing` never clears
    in this suite, since the status read assembles no combined-candidate
    check receipt (documented on `guardShortfallsLocked`, not a bug); the
    section 7 attention surface
...
- Slice 6b bootstrap suites (`go test ./internal/app -run '<name>'`):
  ...
  `TestRetirementUnplacedLaunch` (found/ambiguous/undecodable/unclaimed;
  absent with the claimed process alive or unlistable — outstanding with
  the reason named — and absent with it gone — settled through the
  label-only row, then stopped or retired; manager and worker); `TestFeatureStopResolvesIntegrationInit`
...
- Slice 2b scenario suites (`go test ./internal/app -run '<name>'`):
  ...
  interleavings). These scenarios drive the step the way the loop does
  (`driveIntegrationStep`: `DriveIntegration`, then `DriveIntegrationCheck`
  when it reports the check due). The combined check's own round has its
  own tables:
  - `TestDriveIntegrationReportsTheCombinedCheckDue`: the check is
    reported due, never executed by `DriveIntegration`;
  - `TestIntegrationStepExclusion`: either call in flight leaves the
    other inert, and the round starts nothing past an unresolved
    operation or on an integration not in checking;
  - `TestCombinedCheckInterruptedMidSpawn`: a canceled round leaves the
    execution reconciling under its claim, and stop retires it and rolls
    the candidate back;
  - `TestCombinedCheckIntentRefusedUnderFailureCause`;
  - `TestCombinedCheckShutdownRace`: a terminal-failure shutdown settles
    the running execution and resets the candidate before the round's
    passing outcome, and the round backs off;
  - `TestCombinedCheckPreSpawnInterruption` and its per-task sibling
    `TestFeatureCheckPreSpawnInterruption`: a round canceled during
    materialization or at its revalidation, or refused by a stop, settles
    never spawned, leaves the handle's dispatch scope live, and leaves
    nothing for a stop or a later round to wait on.

  `TestRetireSettledSessions` (dispatch is never
  ...
  the status read);
  `TestResumeFeature` (warm reattach — a pre-claim child's launcher in
  its pane resumes, its recorded pane gone keeps reconciling —
  per-session attestation with continuity evidence, manager
  lineage/succession with historical parents preserved and stale-manager
  verb refusal, changed/unknown instance refusal, Claude-only relaunch);
  the in-flight rule (usecase_resumeinflight_test.go):
  `TestResumeFeatureChildLaunchInFlight` (a live corroborating occupant
  resumes and the loop settles it; a forking wrapper resumes, the loop
  fails it closed and a later resume stays reconciling; no claim yet
  resumes and the loop settles the later claim; an unsettled manager
  returns the run to launching and only its own settlement moves it to
  running; a reconciling manager keeps the run resuming; a held stop
  inspects nothing; a restarted server — fresh occupant, or even the
  claimed process — a restart during the in-flight rule's own
  inspection, an older-format or absent recorded identity, an unknown
  instance, a recorded id gone with the label answering, another process
  in the pane, an empty foreground and a failed pane.open each keep the
  run resuming), `TestResumeFeatureChildRestartAtInspection` and
  `TestResumeFeatureChildRecycledServerIdentity` (the review's
  reproductions: a restored shell answering the recorded id after a
  restart mid-resume, and a foreign shell under the very identity the
  placement recorded, both keep the run resuming),
  `TestResumeFeatureChildUnclaimedLaunchNeedsItsLauncher` (with no claim,
  a login shell, a launcher for another session, the launcher argv on a
  member other than the pane's own process, another hop binary, an extra
  argument, an uninspectable pane process, a frozen command that is not
  a session launch, another or an unknown lifetime each keep the run
  resuming) and `TestResumeFeatureChildUnplacedLaunch` (adoption by label
  then in flight; label and process gone settled through the label-only
  row; a live process pending with the action named);
  `TestPublishRunPresentation` (manager-first tokens, republication,
  rehydration); the launch-ended row (usecase_launchended_test.go):
  `TestLaunchEndedRowSettlesChildren` (implementer, reviewer, held stop,
  idempotent later rounds, a crash after the claim settlement),
  `TestLaunchEndedRowResolvesDeferredClaimWindows`,
  `TestLaunchEndedRowStaysPending` (a live or still-listed pid, a failed
  listing, no inspector, an uninspectable pid, a label still answering, a
  failed label lookup, another inspection error, an empty foreground, a
  superseded binding, and — the continuity conjunct — a renamed pane
  awaiting its deferred native restore after a restart, an unknown,
  absent or older-format lifetime, and a restart between the absence and
  the process observations), `TestLaunchEndedRowFailsTheRunForTheManager`,
  `TestLaunchEndedPredicateGovernsStopAndFailure`,
  `TestLaunchDeadlineYieldsToAClaimThatLanded` (a claim landing between
  the round's no-claim read and the deadline transaction keeps the
  intent pending, in both the solo and the session-keyed driver),
  `TestLaunchEndedRowAfterRestartHasNoAutomaticExit` (resume stays
  resuming with the rename-back action; stop stays stopping with it, and
  finishes only under the placement's lifetime),
  `TestStopAfterRestartConcludesNoAbsence`,
  `TestRetirementAfterRestartConcludesNoAbsence` and
  `TestWorkerExitAfterRestartIsNotObserved` (a renamed pane awaiting its
  deferred restore after a restart: solo stop with and without a claim,
  feature stop of a settled and a claimless child, the accepted-result
  retirement and the live self-exit observation all conclude no absence
  and report the rename-back action, each with its under-one-lifetime
  control),
  `TestResumeFeatureAppliesTheLaunchEndedRow`; its label-only variant
  (usecase_unplacedlaunchended_test.go):
  `TestUnplacedLaunchEndedRowSettlesChildren` (settled with the distinct
  reason, the pane.open failed as dispatched, the child failed with its
  notice, idempotent later rounds, a crash after the row's own
  transaction finished from the resolved intent) and
  `TestUnplacedLaunchEndedRowStaysPending` (a live or still-listed pid, a
  failed listing, no inspector, an uninspectable pid, a failed label
  lookup, a second unresolved launch, an undecodable row, a label still
  answering — bound instead — a binding committed between the
  observation and the settlement, and — the continuity conjunct — a
  renamed pane restored by a restart with a fresh shell, a restored
  harness or its native restore still deferred, a restart with nothing
  answering, an unknown, absent or older-format lifetime, and a restart
  between the label lookup and the process observation) and
  `TestUnplacedLaunchAfterRestartHasNoAutomaticExit` (the residual: stop
  stays stopping and resume stays resuming with the rename-back action, a
  pane renamed back is adopted with its creation-time identity, and a
  hostile label renders escaped in both actions), with the manager's unbound
  exec-pending cases in `TestResumeFeatureBootstrapManagerExecFailure`;
  vanished panes
  (usecase_vanishedpane_test.go): `TestSchedulingPassSurvivesAVanishedPane`,
  `TestPublishRunPresentationToleratesOnlyNotFound`,
  `TestClosePaneNotFoundReobserves`;
  `TestPortSurfaceAllowlist` (the exact method set of
