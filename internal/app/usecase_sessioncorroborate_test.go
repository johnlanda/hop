...
func launchingFeatureRun(t *testing.T, tc *testController, launcherPID int) (runID identity.RunID, managerID identity.SessionID, managerIncarnation identity.IncarnationID, handle app.RunHandle) {
	t.Helper()
	now := tc.Clock.Now()
	...
	tc.Store.LaunchClaims[managerIncarnation] = app.LaunchClaim{
		...
		Executable: "/usr/bin/claude", PID: launcherPID, State: app.LaunchClaimExecPending,
	}
...
func TestCorroborateSessionLaunchesSettlesManagerRunToRunning(t *testing.T) {
	...
	runID, managerID, managerIncarnation, handle := launchingFeatureRun(t, tc, 4242)

	tc.Runtime.InspectPaneFn = func(string) (app.PaneProcess, error) {
		return app.PaneProcess{Foreground: []app.ProcessInfo{
			{PID: 4242, Argv0: "claude", Name: "claude", Argv: []string{"/usr/bin/claude", runID.String()}},
		}}, nil
	}
...
func TestCorroborateSessionLaunchesAlreadyExecedAppliesRunRunning(t *testing.T) {
	tc := newTestController(defaultPolicy())
	runID, _, managerIncarnation, handle := launchingFeatureRun(t, tc, 4242)
