...
- The Phase 3 suites (same command): migration 003 over POPULATED
  ...
  request-ID receipt, then authority, then prior content, one receipt per
  decision — `TestAnswerAcceptanceBothOrders` and `TestAnswerAcceptanceRaced`
  from separate handles, `TestAnswerAfterFailureClosureRefused`,
  `TestHumanAnswerAfterOriginClosure`); the raced contracts across
