...
- `Message.State` is NOT a persisted envelope field (the `messages` row is
  ...
  fetch and every kind of send: a session that has ended (lost or terminated) never is; a
