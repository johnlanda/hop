...
// zero-dependency task given any — since an empty edge set would
// otherwise be indistinguishable from a task with genuinely no
// dependencies and let a caller assert eligibility by omission. Given a
// shape-consistent edges, eligible only when prerequisites accounts for
// EXACTLY the prerequisite IDs edges names — no missing, no foreign (an
// ID edges never named), no duplicate — and every one has reached
// integrated.
